...
colnames(simres) <- c("simulation", "outcome", "coeffthresh", "pthresh", "coeffLSNS")
simres$simulation <- unlist(lapply(1:100, rep, times = 5))
simres$outcome <- rep(c("HCV", "HCVexp", "HCVlin", "HCVthreshexp", "HCVthreshlin"), times = 5)
...
for (n in 1:100) {
  ...
  mod <- lmer(data = df[df$neurocog == 1,], formula = HCV ~ age + gender + LSNS + thresh + (1|ID))
  smod <- summary(mod)
  simres[(1+5*(n-1)),c("coeffthresh", "pthresh")] <- smod$coefficients["thresh", c("Estimate", "Pr(>|t|)")]
  simres[(1+5*(n-1)),c("coeffLSNS", "pLSNS")] <- smod$coefficients["LSNS", c("Estimate", "Pr(>|t|)")]
  modexp <- lmer(data = df[df$neurocog == 1,], formula = HCVexp ~ age + gender + LSNS + thresh + (1|ID))
  smodexp <- summary(modexp)
  simres[(2+5*(n-1)),c("coeffthresh", "pthresh")] <- smodexp$coefficients["thresh", c("Estimate", "Pr(>|t|)")]
  simres[(2+5*(n-1)),c("coeffLSNS", "pLSNS")] <- smodexp$coefficients["LSNS", c("Estimate", "Pr(>|t|)")]
  modlin <- lmer(data = df[df$neurocog == 1,], formula = HCVlin ~ age + gender + LSNS + thresh + (1|ID))
  smodlin <- summary(modlin)
  simres[(3+5*(n-1)),c("coeffthresh", "pthresh")] <- smodlin$coefficients["thresh", c("Estimate", "Pr(>|t|)")]
  simres[(3+5*(n-1)),c("coeffLSNS", "pLSNS")] <- smodlin$coefficients["LSNS", c("Estimate", "Pr(>|t|)")]
  modthreshexp <- lmer(data = df[df$neurocog == 1,], formula = HCVthreshexp ~ age + gender + LSNS + thresh + (1|ID))
  smodthreshexp <- summary(modthreshexp)
  simres[(4+5*(n-1)),c("coeffthresh", "pthresh")] <- smodthreshexp$coefficients["thresh", c("Estimate", "Pr(>|t|)")]
  simres[(4+5*(n-1)),c("coeffLSNS", "pLSNS")] <- smodthreshexp$coefficients["LSNS", c("Estimate", "Pr(>|t|)")]
  modthreshlin <- lmer(data = df[df$neurocog == 1,], formula = HCVthreshlin ~ age + gender + LSNS + thresh + (1|ID))
  smodthreshlin <- summary(modthreshlin)
  simres[(5+5*(n-1)),c("coeffthresh", "pthresh")] <- smodthreshlin$coefficients["thresh", c("Estimate", "Pr(>|t|)")]
  simres[(5+5*(n-1)),c("coeffLSNS", "pLSNS")] <- smodthreshlin$coefficients["LSNS", c("Estimate", "Pr(>|t|)")]
}
