# this script runs (weighted) GAMMS on the imputed datasets
...
diagnostics <- data.frame(matrix(nrow = 5*9*2*2, ncol = 15)) # nrow = n_imputations * n_outcomes * n_covariate_sets * n_models
colnames(diagnostics) <- c("imp",	"outcome",	"covar",	"mod",	"k_age",	"edf_age",   "k_index_age",
                           "p_value_age",	"k_lsns", "edf_lsns",   "k_index_lsns",		"p_value_lsns",
                           "concurvity_worst_para",	"concurvity_worst_age",	"concurvity_worst_lsns")
diagnostics$imp <- rep(1:5, each = 36, length.out = 5*9*2*2)
...
results <- data.frame(matrix(nrow = 5*9*2*2, ncol = 84)) # nrow = n_imputations * n_outcomes * n_covariate_sets * n_models
...
colnames(results) <- c("imp",	"outcome",	"covariates",	"model",	paste0("s_age_", c("edf", "Ref.df", "F", "p-value")),
...
results$imp <- rep(1:5, each = 36, length.out = 5*9*2*2)
...
calculate_GAMMs <- function(outvar, covariates = "min", model = "linear", k_age = 10, k_lsns = 10, icv = 0, 
  ...
  rown <- which(cond1 & cond2 & cond3 & cond4)
  # calculate the (G)AMMs
  if (weighted == 1) {
    res <- gamm4(data = dat, formula = form, random =~(1|ID), REML = F, weights = Weights, family = fam)
  } else {
    res <- gamm4(data = dat, formula = form, random =~(1|ID), REML = F, family = fam)
  }
  # get k-check data and save it in diagnostics df
  kcheck <- k.check(res$gam)
  diagnostics[rown, 5:8] <<- kcheck["s(AGE)",]
  try(diagnostics[rown, 9:12] <<- kcheck["s(LSNS_SUM)",], silent = T) # use try as not every model has smooth LSNS term
  ...
  results[rown, 5:8] <<- sres$s.table[1, ]
  try(results[rown, 9:12] <<- sres$s.table[2, ], silent = T)
...
for(n in 1:5){
  ...
  write.csv(anova_combined, paste0(path, "imp", n, "/anovaresults.csv"), row.names = F)
